...
class ExplorationPolicy(BaseModel):
    ...
    explicit_actions: ClassVar[frozenset[str]] = frozenset(
        {"login", "permission", "submit", "publish", "download"}
    )
    always_forbidden_actions: ClassVar[frozenset[str]] = frozenset(
        {"payment", "delete", "uninstall", "clear_data"}
    )
...
class RunTrace(BaseModel):
    ...
    @model_validator(mode="after")
    def synchronize_frozen_profile(self) -> RunTrace:
        if (
